"""
6. 캠코공매공고 첨부파일 상세조회 (getKamcoPlnmPbctFileInfoDetail)
...
샘플 URL:
http://openapi.onbid.co.kr/openapi/services/KamcoPblsalThingInquireSvc/getKamcoPlnmPbctFileInfoDetail
파라미터: numOfRows, pageNo, PLNM_NO, PBCT_NO
"""
...
def main() -> int:
    ...
    if result_code == "03":
        print("✅ API 호출 성공 (해당 공고에 대한 첨부파일 데이터 없음)")
        return 0
    ...
    if file_list:
        ...
        print(f"✅ 캠코공매공고 첨부파일 상세조회 성공: {count}개 조회 (전체: {total_count})")
        
        first_item = file_list[0] if isinstance(file_list, list) else file_list
        print("\n첨부파일 상세정보 (첫 번째):")
        for k, v in first_item.items():
            print(f"   {k}: {v}")
    else:
        print("✅ API 호출 성공 (첨부파일 데이터 없음)")
